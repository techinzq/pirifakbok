...
(()=>{
  ...
  async function loadPublicStickers(){try{const r=await fetch('/.netlify/functions/sticker-library',{cache:'no-store'});if(!r.ok)return;publicStickers=await r.json();{const cat=document.querySelector('.sticker-tab.active')?.dataset.stickerCat;if(cat==='admin'||cat==='community')renderStickerGrid(cat)}}catch{}}
  async function loadAdminStickerList(){if(!adminAuth)return;const box=document.querySelector('#adminStickerList');if(!box)return;box.innerHTML='<div class="empty" style="grid-column:1/-1">กำลังโหลด...</div>';try{const r=await fetch('/.netlify/functions/sticker-library?admin=1',{headers:authHeaders(),cache:'no-store'});const items=await r.json();box.innerHTML='';if(!items.length){box.innerHTML='<div class="empty" style="grid-column:1/-1">ยังไม่มีสติ๊กเกอร์ของเพจ</div>';return}items.forEach(x=>{const d=document.createElement('div');d.className='admin-sticker-card';d.innerHTML=`<img src="${x.imageUrl}" alt=""><small>${escapeHtml(x.name||'Sticker')}</small><small>${escapeHtml(x.category||'')}</small>`;const del=document.createElement('button');del.textContent='ลบ';del.onclick=async()=>{if(!confirm('ลบสติ๊กเกอร์นี้?'))return;await fetch('/.netlify/functions/sticker-library?id='+encodeURIComponent(x.id),{method:'DELETE',headers:authHeaders()});await loadAdminStickerList();await loadPublicStickers()};d.appendChild(del);box.appendChild(d)})}catch{box.innerHTML='<div class="empty" style="grid-column:1/-1">โหลดไม่สำเร็จ</div>'}}
  document.querySelector('#uploadAdminStickerBtn')?.addEventListener('click',async()=>{const f=document.querySelector('#adminStickerFile').files[0];if(!f)return alert('เลือกไฟล์สติ๊กเกอร์ก่อน');const fd=new FormData();fd.append('image',f);fd.append('name',document.querySelector('#adminStickerName').value||f.name);fd.append('category',document.querySelector('#adminStickerCategory').value);const r=await fetch('/.netlify/functions/sticker-library',{method:'POST',headers:authHeaders(),body:fd});if(!r.ok)return alert('อัปโหลดไม่สำเร็จ');document.querySelector('#adminStickerFile').value='';document.querySelector('#adminStickerName').value='';await loadAdminStickerList();await loadPublicStickers()});

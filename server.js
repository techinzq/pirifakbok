...
async function bridge(req,res,handler){
  try{
    ...
    if(!['GET','HEAD'].includes(req.method)){
      init.body=req.body && req.body.length ? req.body : undefined;
    }
...
app.get("/",(req,res)=>res.json({status:"API is running",message:"fakbok api",storage:process.env.PIRI_DATA_DIR||"temporary"}));
...
app.all("/submission-download",(req,res)=>bridge(req,res,submissionDownload));
app.all("/submission-image",(req,res)=>bridge(req,res,submissionImage));

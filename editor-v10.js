...
(()=>{
  ...
  function renderStickerGrid(cat='community'){const box=document.querySelector('#stickerGrid');box.innerHTML='';const data=stickerData(cat);if(!data.length){box.innerHTML='<div class="empty" style="grid-column:1/-1">ยังไม่มีสติ๊กเกอร์ในหมวดนี้</div>';return}data.forEach(item=>{const [kind,val,label]=item,b=document.createElement('button');b.className='sticker-item';if(kind==='image'){const im=document.createElement('img');im.src=val;im.alt=label||'sticker';b.appendChild(im)}else if(kind==='label'){b.innerHTML=`<span class="sticker-label">${val}</span>`}else b.textContent=val;b.onclick=()=>addSticker(kind,val,label);box.appendChild(b)})}
  ...
  loadPublicStickers().then(()=>renderStickerGrid('community'));ensureState();selected={kind:'text',key:'headline'};syncControls();render();
})();
